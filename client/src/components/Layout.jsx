            ...
            {NAV_LINKS.map(l => (
              <Link
                ...
                style={{
                  ...
                  fontWeight: 600,
                  color: isActive(l.path) ? 'var(--ink)' : 'var(--ink-60)',
                  textDecoration: 'none',
                  borderBottom: isActive(l.path) ? '1.5px solid var(--gold)' : '1.5px solid transparent',
                  paddingBottom: 2,
              ...
              {NAV_LINKS.map(l => (
                <Link
                  ...
                  style={{
                    ...
                    color: isActive(l.path) ? 'var(--gold)' : 'var(--ink)',
                    textDecoration: 'none',
                    borderLeft: isActive(l.path) ? '3px solid var(--gold)' : '3px solid transparent',
                    background: isActive(l.path) ? 'var(--gold-bg)' : 'transparent',
                  }}
                >
                  {l.label}
                  {IC.chevRight}
        ...
        <footer style={{ background: '#111', color: 'rgba(255,255,255,.7)' }} className={!hideBottomNav ? 'footer-has-bottom-nav' : ''}>
          <div style={{ maxWidth: 1200, margin: '0 auto', padding: '48px 5% 40px', display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: 32 }}>
            <div>
              ...
              <p style={{ fontSize: 13, lineHeight: 1.7, color: 'rgba(255,255,255,.5)', maxWidth: 220 }}>
                Fine jewellery crafted with love. BIS Hallmarked gold & certified diamonds.
              </p>
            </div>
            {[
              { title: 'Shop', links: ['Rings', 'Necklaces', 'Earrings', 'Bracelets', 'Pendants'] },
              { title: 'Help', links: ['Track Order', 'Returns', 'Size Guide', 'Care Guide'] },
              ...
              <div key={col.title}>
                ...
                {col.links.map(l => (
                  <div key={l} style={{ fontSize: 13, padding: '5px 0', color: 'rgba(255,255,255,.5)', cursor: 'pointer' }}>{l}</div>
                ))}
              </div>
            ))}
          </div>
          <div style={{ borderTop: '1px solid rgba(255,255,255,.08)', padding: '20px 5%', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12 }}>
            ...
            <div style={{ display: 'flex', gap: 20 }}>
              {['Privacy Policy', 'Terms', 'Shipping Policy'].map(l => (
                <span key={l} style={{ fontSize: 12, color: 'rgba(255,255,255,.35)', cursor: 'pointer' }}>{l}</span>
              ))}
            </div>
          </div>
        </footer>
