// ── Spinner ────────────────────────────────────────────────────────────────
export function Spinner({ size = 24, color = '#B8975A' }) {
  return (
...
export function SectionHeader({ title, subtitle, onViewAll, centered }) {
  return (
    <div style={{
      ...
      <div>
        {subtitle && (
          <div style={{ fontSize: 11, letterSpacing: '.18em', textTransform: 'uppercase', color: 'var(--gold)', fontWeight: 600, marginBottom: 8 }}>{subtitle}</div>
        )}
...
export function ProductCard({ product, onPress, onAddToCart, onWishlist, wishlisted }) {
  ...
  return (
    <div
      onClick={onPress}
      style={{ cursor: 'pointer', flexShrink: 0, width: 170 }}
    >
      <div style={{
...
export function GridProductCard({ product, onPress, onAddToCart, onBuyNow, onWishlist, wishlisted }) {
  ...
  return (
    <div onClick={onPress} style={{ cursor: 'pointer' }}>
      <div style={{ position: 'relative', aspectRatio: '.88', borderRadius: 3, overflow: 'hidden', background: '#f5f5f3', marginBottom: 12 }}>
        <img
        ...
        {discount > 0 && <span className="badge-off" style={{ position: 'absolute', top: 10, left: 10 }}>{discount}% OFF</span>}
        {isNew && !discount && <span className="badge-new" style={{ position: 'absolute', top: 10, left: 10 }}>New</span>}
        {isLow && !isOOS && <span style={{ position: 'absolute', bottom: 10, left: 10, background: 'rgba(180,70,0,.9)', color: '#fff', fontSize: 9, fontWeight: 700, padding: '4px 8px', borderRadius: 1, letterSpacing: '.08em' }}>Only {product.stock} left</span>}
      ...
      <div style={{ fontSize: 10, letterSpacing: '.12em', textTransform: 'uppercase', color: 'var(--ink-40)', marginBottom: 4 }}>{product.category}</div>
      <div style={{ fontSize: 14, fontWeight: 500, color: 'var(--ink)', marginBottom: 6, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{product.name}</div>
      <Stars rating={product.rating} count={product.reviewCount} style={{ marginBottom: 8 }} />
      <div style={{ display: 'flex', alignItems: 'baseline', gap: 8, marginBottom: 12, flexWrap: 'wrap' }}>
        <span style={{ fontSize: 16, fontWeight: 700, color: 'var(--ink)' }}>{fmt(product.price)}</span>
        {product.originalPrice && <span style={{ fontSize: 12, color: 'var(--ink-40)', textDecoration: 'line-through' }}>{fmt(product.originalPrice)}</span>}
      </div>
        ...
        {!isOOS && (
          <button
            ...
            style={{
              ...
              fontSize: 10, fontWeight: 700, letterSpacing: '.08em', textTransform: 'uppercase',
              color: '#fff', fontFamily: "'Jost', sans-serif", borderRadius: 2, transition: 'all .2s',
            }}
    ...
    <div style={{ display: 'flex', alignItems: 'center', gap: 5, ...sx }}>
      <div style={{ display: 'flex', gap: 1 }}>
        {Array.from({ length: 5 }, (_, i) => (
          <span key={i} style={{ color: i < r ? '#B8975A' : 'rgba(28,28,30,.18)', display: 'flex' }}>
            {Icons.star}
          </span>
        ))}
      </div>
...
export function SkeletonCard() {
  return (
    <div>
      <div className="skeleton" style={{ width: '100%', aspectRatio: '.88', borderRadius: 3, marginBottom: 12 }} />
      <div className="skeleton" style={{ height: 10, width: '40%', marginBottom: 6, borderRadius: 2 }} />
      <div className="skeleton" style={{ height: 14, width: '80%', marginBottom: 8, borderRadius: 2 }} />
      <div className="skeleton" style={{ height: 18, width: '50%', marginBottom: 12, borderRadius: 2 }} />
      <div className="skeleton" style={{ height: 38, width: '100%', borderRadius: 2 }} />
    </div>
  )
}
